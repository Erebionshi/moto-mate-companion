...
export function AIChatSection({ vehicle, statuses, chat, setChat }: { vehicle: Vehicle; statuses: ServiceStatus[]; chat: ChatMsg[]; setChat: (m: ChatMsg[]) => void }) {
  ...
  async function ask(q: string) {
    ...
    const thinkingMsg: ChatMsg = { id: "thinking", role: "bot", text: "…", ts: Date.now() + 1 };
    setChat([...chat, userMsg, thinkingMsg].slice(-21));
    setInput("");
    setIsAsking(true);

    try {
      const result = await apiClient.ai.ask({
        question: q,
        brand: vehicle.brand,
        model: vehicle.model,
        year: vehicle.year,
        odometer: vehicle.odometer,
        overdueItems: ctx.overdue,
        dueSoonItems: ctx.dueSoon,
      });
      const botMsg: ChatMsg = { id: uid(), role: "bot", text: result.answer, ts: Date.now() };
      setChat([...chat, userMsg, botMsg].slice(-20));
    } catch {
      // Gemini unavailable — fall back to built-in responses
      const botMsg: ChatMsg = { id: uid(), role: "bot", text: aiAnswer(q, ctx), ts: Date.now() };
      setChat([...chat, userMsg, botMsg].slice(-20));
    } finally {
      setIsAsking(false);
    }
    ...
    <div className="px-4">
      ...
      <div className="card-mn p-3">
            ...
            {chat.slice().reverse().slice(0, 10).reverse().map((m) => (
              <div key={m.id} className={`rounded-md p-2 text-xs ${m.role === "user" ? "bg-white/10 text-white" : "bg-[#0a0a0a] text-[#ddd]"}`}>
                <div className="whitespace-pre-line">
                  {m.id === "thinking" ? <span className="animate-pulse opacity-60">Thinking…</span> : m.text}
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="mt-3 flex gap-2">
          <input className="input-mn flex-1 px-3 py-2 text-sm" placeholder="Ask about your motorcycle…" value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && ask(input)} disabled={isAsking} />
          <button onClick={() => ask(input)} disabled={isAsking} className="rounded-md bg-white px-3 text-black disabled:opacity-40"><Send size={14} /></button>
        </div>
      </div>
    </div>
  );
